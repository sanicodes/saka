...
import {
  MAX_ROOMS,
  ROOM_CREATE_COOLDOWN_MS,
  MAX_PASSWORD_LEN,
} from '../shared/constants.js';

// normalize a client-supplied password to a stored value (null = no password)
...
export function wireSockets(io) {
  ...
  // leave the room this socket is in, destroy it if it empties
  ...
  io.on('connection', (socket) => {
    ...
    function doJoin(socket, room, playerName, cb) {
      leaveRoom(socket); // only one room at a time
      socket.leave('lobby');
      socket.join(room.id);
    ...
    socket.on('room:settings', (settings) => {
      ...
      if (room && settings && typeof settings === 'object') {
        room.setSettings(socket.id, settings);
      }
    });
